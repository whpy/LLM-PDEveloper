# The scheme of the creepy  advection-diffusion-reaction model in LBM
...
Here, $\phi$ is a scalar, which represents heat or solute; $\alpha$ is a constant called diffusion coefficient; vector $\boldsymbol{u}$ is the velocity of flow. We usually consider the $\boldsymbol{u}$ as a constant vector field; $R(\phi)$ denotes the effect of reation, in this paper $R(\phi) = \frac{k|\boldsymbol{u}|\phi}{K+\phi},$, $k$ is a diffusive coefficient.
...
0. The reaction term $R(\phi(\boldsymbol{x},t))$, where $k=0.005$;
