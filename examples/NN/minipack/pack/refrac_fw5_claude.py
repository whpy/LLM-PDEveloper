...
class ver_result(BaseModel):
    result: bool = Field(description="The code and the algorithm is consistent or not.")
...
# from verifier1 to generator
def transition1(state:GraphState):
    if not state['gen_ver'].result:
    # if not False:
        return 'generator'
    else:
        return 'end'

# from verifier2 to corrector
def transition2(state:GraphState):
    if not state['corr_ver'].result:
    # if not False:
        return 'corrector'
    else:
        return 'end'
...
code_llm = ChatAnthropic(temperature=0.05, model=code_llm_id)
...
corr_llm = ChatAnthropic(temperature=0.05, model=code_llm_id)
...
'''
### verifier agent
ver_llm_id = 'claude-3-5-sonnet-20241022'
ver_llm = ChatAnthropic(temperature=0.05, model=code_llm_id)
print("verifier_id",ver_llm_id)
...
ver_chain_oai = ver_prompt | ver_llm.with_structured_output(ver_result)
ver_sys_prompt_template = '''
Setting:
Role: You are now an expert in python and Jax. You will receive a code generated by other LLM. The code is generated to implement algorithm in the given paper. Your responsibility is to\
check whether the code is consistent with the given paper.
Matterials: Here is the generated codes:\n\
{paper}
\n\
\n\
Here is the paper of the algorithm:\n\
{code}
\n\
\n\
Output format:
Your final answer should be 'True' or 'False'. And it should be like '<VERIFIER> True </VERIFIER>' or '<VERIFIER> False </VERIFIER>'.
'''

# ver_format_llm_id = 'gpt-4o'
# print("verifier_format_id",ver_format_llm_id)
# ver_format_llm = ChatOpenAI(temperature=0.2, model=ver_llm_id)
# ver_prompt = ChatPromptTemplate.from_messages(
#     [
#         ("placeholder", "{messages}"),
#     ]
# )
# ver_chain_oai = ver_prompt | ver_llm
# ver_sys_prompt_template = '''
# Setting:
# Role: You are now an expert in python and Jax. You will receive a code generated by other LLM. The code is generated to implement algorithm in the given paper. Your responsibility is to\
# check whether the code is consistent with the given paper.
...
def verify1(state:GraphState):
    messages = state["generation_messages"]
    paper = state['paper']
    sys_prompt = ver_sys_prompt_template.format(paper=paper, code=state['first_generation'])
    ver_r = ver_chain_oai.invoke({"placeholder": "you are a good assistant.", 
                               "messages": [
                                   ("user",sys_prompt)]
                                   }
                            )
    print("verify 1: ",ver_r)
    return {'gen_ver':ver_r}

def verify2(state:GraphState):
    paper = state['paper']
    sys_prompt = ver_sys_prompt_template.format(paper=paper, code=state['correction'])
    ver_r = ver_chain_oai.invoke({"placeholder": "you are a good assistant.", 
                               "messages": [
                                   ("user",sys_prompt)]
                                   }
                            )
    print("verify 2: ",ver_r)
    return {'corr_ver':ver_r}
...
workflow.add_node("verifier1", verify1)
workflow.add_node("verifier2", verify2)

workflow.add_edge(START, "generator")
workflow.add_edge('generator', 'verifier1')
...
workflow.add_edge('corrector','verifier2')
workflow.add_conditional_edges(
    'verifier2',
    transition2,
    {
...
workflow.add_edge("verifier2", "tester")
