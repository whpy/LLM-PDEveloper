...
### Boundary Condition
For the boundary condition of ADM, we introduce constant scalar boundary condition. It describes that the value of scalar at the boundary is a given constant. We use the same symbols as before, the formulation of this boundary is: 
$$
f_{1} = C(\omega_{3} + \omega_{1}) - f_{3}\\
f_{2} = C(\omega_{4} + \omega_{2}) - f_{4}\\
f_{3} = C(\omega_{1} + \omega_{3}) - f_{1}\\
f_{4} = C(\omega_{2} + \omega_{4}) - f_{2}\\
f_{5} = C(\omega_{7} + \omega_{5}) - f_{7}\\
f_{6} = C(\omega_{8} + \omega_{6}) - f_{8}\\
f_{7} = C(\omega_{5} + \omega_{7}) - f_{5}\\
f_{8} = C(\omega_{6} + \omega_{8}) - f_{6}\\
$$
The constant $C$ denotes the constant scalar(temperature or others) on the boundary.
...
2. Apply the Direchlet BC and other BCs that are post-collision; 
...
2. BCs: the value on left side wall is constant 1.0, with the others are all 0.0;
