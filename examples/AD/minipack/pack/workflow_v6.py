...
Gem_key = "AIzaSyBmfDT0Ty6BHkGDNTBupjOpxoNlIO3tbT8"
DP_key = os.environ['DPSK_API_KEY']

from typing import List, TypedDict
from langgraph.graph import END, StateGraph, START

print("this version is for physical evaluation!!!!!!!!!!!!")
# Data model
...
def merge(ori_code:code_cache, add_code:code_cache)->(bool, code_cache):
    ...
    if len(add_code.files) != len(add_code.codes):
        ...
        gen_wrapper = "\n\n#<BEG> LLM added buffer <\BEG>\n\n{generation}\n\n#<END> LLM added buffer <\END>\n\n"
...
# llm_fmt = ChatOpenAI(temperature=0.7, model=fmt_llm_id, max_tokens=4096*2)

fmt_llm_id = "claude-3-5-sonnet-20241022"
print("fmt llm id: "+fmt_llm_id)
llm_fmt = ChatAnthropic(temperature=0.1, model=fmt_llm_id, max_tokens=4096*2)
...
code_llm_id = "gemini-2.0-flash-exp"
# code_llm_id = "deepseek-chat"
print("code llm card: "+code_llm_id)
code_llm = ChatGoogleGenerativeAI(
    model="gemini-2.0-flash-exp",
    temperature=0.1
)
# code_llm = ChatOpenAI(temperature=1, model=code_llm_id,openai_api_key=OA_key) # o1-preview not supported the max_tokens

# code_llm_id = "claude-3-5-sonnet-20241022"
# print("code llm card: "+code_llm_id)
# code_llm = ChatAnthropic(temperature=1, model=code_llm_id, max_tokens=4096*2) 
...
### corrector llm
corr_llm_id = "gemini-2.0-flash-exp"
...
# corr_llm = ChatOpenAI(temperature=1, model=corr_llm_id, openai_api_key=OA_key) # o1-preview not supported the max_tokens

# corr_llm_id = "claude-3-5-sonnet-20241022"
# print("corr llm: "+corr_llm_id)
# corr_llm = ChatAnthropic(temperature=1, model=corr_llm_id) # o1-preview not supported the max_tokens
...
Matterials:\n\
This is the code base called XLB user now could only see: {ori_code_mannual} \n\n\
...
{err_msg}\n\
    ...
    3. Instruct user about how to modify code base. You could see the code blocks wrapped by "#<BEG> LLM added buffer <\BEG> #<END> LLM added buffer <\END>". \
All these codes would not be seen by user, so for the llm generated code without error, you should also generate for user. \n\
...
def generate(state:GraphState):
    ...
    messages += [
            (
                "user",
                code_sys_prompt_template.format(paper=state["paper"], src_code = shorter_src_code),
            )
        ]
    raw_code =  code_gen_chain_oai.invoke({ 
                               "messages": [
                                   ("user",code_sys_prompt_template.format(paper=state["paper"], src_code = shorter_src_code))]
                                   })
    print("first gen pass")
    with open("st_gen.md","w") as f:
        print("corrrrrrrrrrrrrrrrrrrrrrrr___________\n", raw_code.content)
...
def correct(state:GraphState):
    ...
    Ori_code_mannual = generate_mannual(state["original_src_code"])
    sys_prompt = corrector_sys_prompt_template.format(modified_src_code=mod_code_mannual, err_msg = ts_err, ori_code_mannual=Ori_code_mannual,paper=state["paper"])
    req = sys_prompt
...
def test2(state:GraphState):
    refresh()
    import os 
    os.system("rm *vtk *png")
    ts_code_cache = state["test_code_cache"]
    code_cache2env(ts_code_cache)
    import numpy as np
    import subprocess
    ...
    try:
        result = subprocess.run(['python3','pytest.py'],
                                capture_output = True,
                                text = True,
                                check = True)
        print("output: ", result.stdout)
        phys_eval = phytest()
        if np.abs(phys_eval) > 0.0001:
            print(phys_eval)
            return {"test_pass":False, "test_error":"error too big"}
        else:
            print(phys_eval)
            return {"test_pass":True}
    except subprocess.CalledProcessError as phye:
        print("Error: ", phye.stderr)
        backup_media("./.testbackup/", state["correction_iterations"])
        print(str(phye.stderr))
        return {"test_pass":False, "test_error":str(phye.stderr)}
...
workflow.add_node("tester", test2)
workflow.add_node("corrector", correct)

workflow.add_edge(langgraph.graph.START, "generator")
...
with open("shorter_src_code.md") as f:
    shorter_src_code = f.read()
...
solution = app.invoke({"original_src_code":original_src_code,
 "paper":paper, "shorter_src_code":shorter_src_code, 
...
# code_cache2env(a)
# test(solution)
# print(solution["first_generation"])
# print(type(solution["original_src_code"]))
# print(type(solution["first_generation"]))
# print(solution["original_src_code"].files.index("src/base.py"))
# b = merge(solution["original_src_code"], solution["first_generation"])
# print(b)
# print("original_src_code", solution["original_src_code"])
# print('b \n')
# print(b)
