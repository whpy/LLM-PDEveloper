...
Materials:\n\
Here is the XLB library:\n {ori_code_mannual} \n\
Task:\n\
You will receive a suggestion about code modification from an externel LLM. Your task is to transform the received textual suggestion into a specialized class format that comprises two lists: one specifying the filenames to be modified and the other containing the corresponding code segments to be appended. For instance, if the file `test0.py` requires the addition of `print("hello world!")` and `src/utils.py` needs `print("byebye")` appended, your output should be:\n\
```python
filenames = ["test0.py", "src/utils.py"] 
code_segments = ['print("hello world!")', 'print("byebye")']
``` \n\ 
Here are some notes:\n\
1. Always specify the complete paths for the files to be modified. For instance, use `src/base.py` instead of `base.py`. \n\

2. Ensure that your result includes a `test.py` file for validation purposes. This file must contain meaningful content and should not be empty.\n\

3. Refrain from including extraneous strings, such as multiple newline characters (`"\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n\\n"`), as they can cause the framework to fail.\n\

Below is a suggestion generated by another LLM: \n\
{generation}
""",
        ),
...
code_llm_id = 'gemini-2.0-flash-thinking-exp'
...
You will generate a code for a PDE solver and/or its associated boundary conditions (BCs) within the XLB library---a Lattice Boltzmann Method (LBM) framework built with Python and JAX (a Python library developed by Google). 
\n\ 
Materials:\n\
The XLB library is:\n\
{src_code}\n\
...
Workflow for applying your code:\n\
Step 1. As outlined in the section "Role", your task is to generate code for a PDE solver and/or BCs. Submit your code in the specified format outlined in the "Format of submission" section below. Your submission should detail the files to be modified and include the **complete** code blocks to be appended to the end of each specified file.\n\
Step 2. The system will automatically append the code from your submission to the end of the specified files, as per your instructions in your submission. 
\n\
Steps of task:\n\
Your task involves three steps: \n\
    Step 1. Develop the solver or BCs as described in the given paper in the "Materials" section. Ensure that your implementation aligns with the existing codebase's structure and style.\n\
    Step 2. Based on the test case in the paper, generate a Python file named "test.py". This script is used to verify the functionality of your implementation. It should produce both `.vtk` and `.png` files for visualization, following the output methods used in Python file "test_template.py"\n\
    Step 3. Submit your code following the format specified in the "Format of submission" section below. \n\
\n\
Format of submission:\n\
    1. Your submission include the generated code should clearly list each file that requires modification. For each file, provide the **complete** code to be appended at the end.\n\
       **Example:**
   ...
   - **Action:** Append the following code to the end of `src/models.py`:
    ...
    3. Your submission MUST create a Python file called "test.py". User validate the your code by executing "test.py". Refer to the existing `test_template.py` for guidance. Ensure that `test.py` is capable of generating both `.vtk` and `.png` files during execution.\n\ 
...
\n\
Here are some notes for coding based on the errors you have made:\n\
{coding_notes}
'''
...
corr_llm_id = 'gemini-2.0-flash-thinking-exp'
corr_llm = ChatGoogleGenerativeAI(
    model=corr_llm_id,
    temperature=0
)
...
You will analyze and correct provided code snippets for a PDE solver within the XLB library---a Lattice Boltzmann Method (LBM) framework built with Python and JAX (a Python library developed by Google). 
\n\ 
...
Automatic workflow:\n\
The workflow for applying your correction includes two steps: \n\
Step 1. All the code segments from other LLM between ”#<BEG> LLM added <\BEG>“ and “#<END> LLM added <\END>” would be deleted.\n\ 
Step 2. Submit your code for correction as detailed in the "Role" section, using the format specified in the "Format of submitted code" section below. Your submission MUST include all necessary code modifications and clearly indicate which files require updates. \n\
Step 3. Based on your submitted code, user  manually append your provided code to the end of the specified files.\n\

Steps of task:\n\
    Your task consists three steps: \n\
    Step 1. Generate a complete code that implements the algorithm, adapting the code from the external LLM while integrating lessons learned from the compilation errors. \n\ 
    Step 2. According to the test case described in the paper and the code generated in Step 1, generate a new test file named "test.py" that allows the user to validate your code. \n\
    Step 3. Provide a brief summary of your correction, formatted according to the guidelines detailed in the "Format of Correction" section below. \n\

Format of submitted code:\n\
    1. Your solution should Clearly list each file that requires modification. For each file, provide the **complete** code block to be appended at the end.\n\
       **Example:**
   ...
   - **Action:** Append the following code to the end of `src/models.py`:
     ...
     class ExampleSolver:
         # Complete implementation of the ExampleSolver class
     ```\n\
    2. your correction must include a file called "test.py". The user would execute it to validate the code you generate.\n\ 
\n\
Noted:\n\
Some IMPORTANT POINTS you need to keep in mind:\n\
...
def generate(state:GraphState):
    ...
    print(code_sys_prompt_template.format(paper=state["paper"], src_code = shorter_src_code, coding_notes=state["coding_notes"]))
    ...
    with open("st_gen.md","w") as f:
        f.write("output tokens: "+"\n" + str(raw_code.usage_metadata['output_tokens']) + "\n" + "total_tokens" + str(raw_code.usage_metadata['total_tokens']) +raw_code.content)
    # format the results
...
def correct(state:GraphState):
    ...
    print(r_code.usage_metadata['total_tokens'])
    with open("correct_record.md","a") as f:
        f.write("\n iters "+str(state["correction_iterations"]) + "\n" + "output tokens:\n" + str(r_code.usage_metadata['output_tokens']) + "\n" + "total_tokens" + str(r_code.usage_metadata['total_tokens']) +" \n"+r_code.content)
