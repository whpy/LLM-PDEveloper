...
code_llm_id = 'qwen-plus'
code_llm = BaseChatOpenAI(model=code_llm_id,openai_api_key=qwen_key, openai_api_base='https://dashscope.aliyuncs.com/compatible-mode/v1')
...
Role:\n\
You are now an expert in Computational Fluid Mechanic and LBM. You are also familiar with Jax (a library developed by Google). You are now helping user to implement a new solver or boundary conditions based on a given LBM code base (XLB)\n\
Here is the source codes of XLB and the location of each file:\n\
{src_code}\n\
...
Tasks:\n\
You have 3 tasks: \n\
    1. Implement the new solver or boundary conditions mentioned in the paper;\n\
    2. Generate a test file to test the codes you generate.\n\
    3. Tell user that how to merge the codes you generated into the XLB \n\
\n\
Format of answer:\n\
    1. Your answer should  include the files to be modified and the complete codes.\n\
    For example, if  you generate a solver called "examplesolver" which would be added at the end of "models.py". Your answer must be like 
    ...
    ```"
    2. The codes of new solver and boundary condition you generate would be directly pasted at the end of the files you mention (i.e. the 'added file'). So please make the incremental update.\n\
    3. You should also provide a file called "test.py". We use it to validate your codes added in XLB. You could refer to the "test_template.py" file to write test file.\n\ 
\n\
Noted:\n\
Some IMPORTANT POINTS you need to keep in mind:\n\
    ...
    5. The new code you generate would be directly pasted at the end of the original code, so try to do incremental correction. And clearly point out that which file to be modified.\n\
    6. Your suggestions must not create new files\n\
\n\
...
corr_llm_id = 'qwen-plus'
corr_llm = BaseChatOpenAI(model=corr_llm_id,openai_api_key=qwen_key, openai_api_base='https://dashscope.aliyuncs.com/compatible-mode/v1')
...
You are also a super expert in Jax coding.\n\

Matterials:\n\
Here is a paper, user wants to implement the algorithm in it:\n{paper}\n\n\
Here is the code base (XLB) with additional codes generated by other LLM. The codes wrapped by the markers '#<BEG> LLM added <\BEG>' and '#<END> LLM added <\END>' is from other LLM. They are generated to implement the algorithm in the paper. They don't belong to the XLB.\n\
{modified_src_code}\n\
Here is the error message while running the test file "test.py", generated by other LLM:\n\
{err_msg}\n\
Tasks:\n\
    ...
    3. Tell the user that how to correct the code base. You could see the code blocks wrapped by the markers "#<BEG> LLM added <\BEG>" and "#<END> LLM added <\END>". \
These codes are generated by other LLM with errors while running (given in the matterials). you should regenerate the complete and correct codes to implement the solver or boundary condition in the given paper (in matterials). All the codes between "#<BEG> LLM added <\BEG>" and "#<END> LLM added <\END>" would be deleted first. Then the codes you generated would be merged into the code base.\n\
...
Output format:\n\
    ...
    3. You should also provide a file called "test.py". We use it to validate your codes merged into XLB. You could refer to the "test_template.py" to write the test file.\n\ 
\n\
Noted:\n\
Some IMPORTANT POINTS you need to keep in mind:\n\
    1. Your codes should not change the original codebase too much.\n\
    2. Your codes MUST be complete, because user has no knowledge about LBM. and the style should be similar to the XLB lib.\n\
    3. Your generations should include the test file named "test.py". User would use it to test the new solver and boundary conditions you implement.\n\
    4. Your test.py should output some .vtk files and .png image files to help user check your code.\n\
