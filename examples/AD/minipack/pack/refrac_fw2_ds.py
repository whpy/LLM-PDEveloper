...
Task:\n\
Your task is to transform the received textual suggestion into a specialized class format that comprises two lists: one specifying the filenames to be modified and the other containing the corresponding code segments to be appended. For instance, if the file `test0.py` requires the addition of `print("hello world!")` and `src/utils.py` needs `print("byebye")` appended, your output should be:\n\
```python
...
fmt_llm_id = "gpt-4o" # only 4o could be adopted
print("fmt llm id: "+fmt_llm_id)
llm_fmt = ChatOpenAI(temperature=0.05, model=fmt_llm_id)
...
\n\ 
Materials:\n\
Here is the source code of XLB library:\n\
{src_code}\n\
...
Step 2: As detailed in the "Role" section, your task is to correct the provided code snippets for the PDE solver. Submit your correction using the format specified in the "Format of code" section below. Your correction includes all necessary code modifications and clearly indicate which files require updates. The user will manually append your provided code to the end of the specified files.\n\
Steps of task:\n\
